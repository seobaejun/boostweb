# Firebase Firestore 보안 규칙 배포 가이드

## 문제: Firestore에 데이터가 저장되지 않음 (오류 메시지 없음)
...
4. **규칙 배포 (중요!)**
   ...
   - 배포 후 "배포 완료" 메시지가 표시됩니다

5. **확인**
   - 배포 후 회원가입을 다시 시도하세요
   - 브라우저 콘솔(F12)에서 오류 메시지 확인

### 방법 2: Firebase CLI로 배포 (선택사항)
...
3. `15. ✅ Firestore set() 완료` - set()이 성공했는지
4. `15-2. 즉시 조회 결과: ✅ 존재함` - 문서가 저장되었는지
